...
namespace Yoda.Net.Networking
{
    ...
    public class SocketManager 
    {
        ...
        public void SendEnc(int id)
        {
            PiggStream stream = new PiggStream();
            Header header = new Header { callId = id, type = TYPE_ENC, length = 0, packetId = 0 };
            header.write(stream);
        ...
        {
            PiggStream stream = new PiggStream();
            Header header = new Header { callId = id, type = TYPE_ID, length = 0, packetId = 0 };
            header.write(stream);
            SendData(stream.toArray());
        ...
        {
            PiggStream stream = new PiggStream();
            Header header = new Header { callId = 0, type = TYPE_PING, length = 0, packetId = 0 };
            header.write(stream);
            SendData(stream.toArray());
